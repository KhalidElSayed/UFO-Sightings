...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    if(_minThumbOn){
       ...
       [leftSliderValueController updateSliderValueTo:floor(selectedMinimumValue)];

        [leftPopOver presentPopoverFromRect:_minThumb.frame inView:self permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
    }
    if(_maxThumbOn){
        ...
        [rightSliderValueController updateSliderValueTo:floor(selectedMaximumValue)];
        [rightPopOver presentPopoverFromRect:_maxThumb.frame inView:self permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
    }
    [self updateTrackHighlight];
    [self setNeedsLayout];
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    if ([rightPopOver isPopoverVisible]) {
        [rightPopOver dismissPopoverAnimated:NO];
    }
    
    if([leftPopOver isPopoverVisible])
        [leftPopOver dismissPopoverAnimated:NO];
